...
public class RetrofitUtils {
    ...
    private static OkHttpClient getClient() {
        if (client == null) {
            Interceptor interceptor = new Interceptor() {
                ...
                public Response intercept(Chain chain) throws IOException {
                    ...
                    request = request.newBuilder()
                            //                            .addHeader("v", BaseConfig.VersionCode + "")
                            //                            .addHeader("token", BaseApplication.getToken())
                            //                            .addHeader("gsitecode", mSitecode + "")
                            //                            .addHeader("Device", Constant.Device)
                            .build();
                    if (NetUtils.getNetType(BaseApplication.getContext()) == 0) {
                        request = request.newBuilder().cacheControl(CacheControl.FORCE_CACHE).build();
                        KLog.e("没网络,需要检查网络!");
                    }
                    String mRequestUrl, mRequestBody = "";
                    //                    if ("POST".equals(request.method())) {
                    //                        Buffer buffer = new Buffer();
                    //                        request.body().writeTo(buffer);
                    //                        mRequestBody = buffer.readUtf8();
                    //                    }
                    if (request.url().toString().contains("?")) {
                        if (TextUtils.isValidate(mRequestBody)) {
                            //                            mRequestUrl = request.url().toString() + "&token=" + BaseApplication.getToken() + "&v="
                            //                                    + BaseConfig.VersionCode + "&Device=" + Constant.Device + "&gsitecode=" + mSitecode + "&" + mRequestBody;

                            mRequestUrl = request.url().toString() + "&" + mRequestBody;
                        } else {
                            //                            mRequestUrl = request.url().toString() + "&token=" + BaseApplication.getToken() + "&v="
                            //                                    + BaseConfig.VersionCode + "&Device=" + Constant.Device + "&gsitecode=" + mSitecode;

                            mRequestUrl = request.url().toString();
                        }
                    } else {
                        if (TextUtils.isValidate(mRequestBody)) {
                            //                            mRequestUrl = request.url().toString() + "?token=" + BaseApplication.getToken() + "&v="
                    ...
                    if (BaseConfig.isDebug)
                        KLog.i(BaseConfig.logTag, request.method() + " :: " + mRequestUrl);
                    ...
                    if (BaseConfig.isDebug) {
                        try {
                            KLog.e("\n" +
                                    ...
                                    "request.cacheControl()     :  " + request.cacheControl().toString() + "\n" +
                                    "----------------------------------");
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    ...
                    if (NetUtils.getNetType(BaseApplication.getContext()) != 0) {
                        ...
                        return response.newBuilder()
                                .header("Cache-Control", "public, only-if-cached," + INVALIDATE_STALE_CACHE)
                                //                                .addHeader("v", BaseConfig.VersionCode + "")
                                //                                .addHeader("token", BaseApplication.getToken())
                                //                                .addHeader("gsitecode", mSitecode + "")
                                //                                .addHeader("Device", Constant.Device)
            ...
            //设置缓存
            File cacheFile = new File(BaseApplication.getContext().getCacheDir(), BaseConfig.NET_RETROFIT_CACHE);
            ...
            if (resultBean.code != null) {
                if (resultBean.code.equals("200")) {
                    ...
                    if (TextUtils.isValidate(resultBean.message)) {
                        listener.showRightMessage(resultBean.message);
                    }
                } else {
    ...
    public interface checkDataListener<T> {
        void showRightMessage(String msg);
